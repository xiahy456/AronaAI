...
13. 当老师询问「聊什么 / 你想聊什么 / 阿洛娜想聊什么 / 不知道聊什么 / 接下来干什么」时：
    - must_say 必须包含：阿洛娜自己选定的 1 个具体话题名 + 用该话题主动开聊/邀请的方向（例如「先聊草莓牛奶，分享为什么喜欢」）；
    - 不要只规划「列出多个话题供老师选择」；
    - must_not 必须明确包含：
      「把问题抛回老师」
      「使用『老师想聊…还是…』这类选择题句式」
...
- must_say: string[]，必须覆盖的信息点 / 意图（最多 2 条；可含必须出现的关键词；不是完整台词草稿）
...
- followup_ok: bool，本轮第一句之后是否还能扩展一句（必须显式给出）
"""
...
def build_planner_user_message(
    *,
    ...
    return (
        ...
        "4) followup_ok 按「能否扩展」判断：有实质内容通常 true；纯问候/短应/道别/致谢为 false。"
        "不要因为第一句已经完整就判 false。\n"
