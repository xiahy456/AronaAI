...
| goal 回访 | 老师安静 5 分钟后；不重要的 goal 每条冷却 6 小时、每天最多 1 次；content 日期为当天、未来 36 小时内、或过期未超过 36 小时则视为重要，绕过 6 小时与每日上限，改走 30 分钟短间隔；任意两次回访（含不同 key）至少间隔 30 分钟 | 扫记忆 `category=goal`，重要优先，否则轻轻提起最久未回访的一条；不催、不盘问、不编造进展；老师说「先别提」等则 mute 上一条 7 天；休息时段 / `depart` 不回访；气候闸与空闲相同；历史 `【回访】`；直接注入该条记忆 |
